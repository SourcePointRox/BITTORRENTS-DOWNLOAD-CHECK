...
const App = {
  setup() {
    ...
    /* 轮询 */
    function refreshStats() { fetch('/api/stats').then(r=>r.json()).then(applyStats).catch(()=>{}); }
    function refreshCharts() { fetch('/api/charts?mins=' + curMins.value).then(r=>r.json()).then(applyCharts).catch(()=>{}); }
    function refreshTrackers() { fetch('/api/trackers').then(r=>r.json()).then(d => { trkList.value = d.list || []; }).catch(()=>{}); }
    function refreshNodes() { fetch('/api/nodes').then(r=>r.json()).then(d => { nodes.value = d.nodes || []; }).catch(()=>{}); }
  ...
  template: '<div class="fade-in">' +
    ...
    '<div style="max-width:1680px;margin:0 auto;padding:14px">' +
      '<div style="display:grid;gap:12px;grid-template-columns:repeat(8,1fr)">' +
      ...
      '</div>' +
      '<div style="display:grid;gap:12px;grid-template-columns:2fr 1fr;margin-top:12px">' +
        '<div class="card"><div class="card-title"><span>采集趋势 · 多来源堆叠</span><span style="display:flex;gap:4px"><button data-mins="15" @click="setMins(15)" :style="curMins===15 ? \'background:#238636;color:#fff;border-color:#238636\' : \'background:transparent;color:#7d8a99;border-color:#232c38\'" style="padding:3px 10px;border-radius:4px;border:1px solid;cursor:pointer;font-size:11px">15m</button><button data-mins="60" @click="setMins(60)" :style="curMins===60 ? \'background:#238636;color:#fff;border-color:#238636\' : \'background:transparent;color:#7d8a99;border-color:#232c38\'" style="padding:3px 10px;border-radius:4px;border:1px solid;cursor:pointer;font-size:11px">1h</button><button data-mins="180" @click="setMins(180)" :style="curMins===180 ? \'background:#238636;color:#fff;border-color:#238636\' : \'background:transparent;color:#7d8a99;border-color:#232c38\'" style="padding:3px 10px;border-radius:4px;border:1px solid;cursor:pointer;font-size:11px">3h</button><button data-mins="360" @click="setMins(360)" :style="curMins===360 ? \'background:#238636;color:#fff;border-color:#238636\' : \'background:transparent;color:#7d8a99;border-color:#232c38\'" style="padding:3px 10px;border-radius:4px;border:1px solid;cursor:pointer;font-size:11px">6h</button><button data-mins="720" @click="setMins(720)" :style="curMins===720 ? \'background:#238636;color:#fff;border-color:#238636\' : \'background:transparent;color:#7d8a99;border-color:#232c38\'" style="padding:3px 10px;border-radius:4px;border:1px solid;cursor:pointer;font-size:11px">12h</button><button data-mins="1440" @click="setMins(1440)" :style="curMins===1440 ? \'background:#238636;color:#fff;border-color:#238636\' : \'background:transparent;color:#7d8a99;border-color:#232c38\'" style="padding:3px 10px;border-radius:4px;border:1px solid;cursor:pointer;font-size:11px">24h</button></span></div><div style="position:relative;height:240px;margin-top:6px"><canvas id="rateChart"></canvas></div><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:6px;font-size:11px"><span v-for="s in SRC_DEFS" :key="s.key" style="display:flex;align-items:center;gap:4px"><i :style="{ background: s.color, width: \'10px\', height: \'10px\', borderRadius: \'2px\', display: \'inline-block\' }"></i>{{ s.label }}</span></div></div>' +
      ...
      '</div>' +
      '<div style="display:grid;gap:12px;grid-template-columns:repeat(3,1fr);margin-top:12px">' +
      ...
      '</div>' +
      '<div style="display:grid;gap:12px;grid-template-columns:repeat(4,1fr);margin-top:12px">' +
