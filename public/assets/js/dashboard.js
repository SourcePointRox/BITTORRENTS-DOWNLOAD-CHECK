'use strict';
/* 监控 WebUI Vue 3 应用 —— 独立 JS 文件，避免模板字面量转义问题。
   此文件由 monitor.js 的 dashboardHtml() 通过 <script src> 引用。 */
...
const App = {
  setup() {
    ...
    const sizeRate = computed(() => stats.value.meta.total > 0 ? stats.value.meta.withSize * 100 / stats.value.meta.total : 0);
    ...
    const coldRate = computed(() => {
      ...
      if (!cs) return 0;
      const main = cs.total + (cs.pending || 0); // coldTotal + pending = mainTotal
      if (!main) return 0;
      return Math.min(100, Math.round(cs.total * 100 / main));
    });
    ...
    function ensureMemChart() {
      ...
      memChart = new Chart(el.getContext('2d'), {
        type: 'line',
        data: { labels: ['0'], datasets: [{ data: [0], borderColor: '#d29922', backgroundColor: 'rgba(210,153,34,0.15)', fill: true, pointRadius: 2, pointHoverRadius: 4, borderWidth: 1.5, tension: 0.3 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { callbacks: { title: () => '', label: (ctx) => fmtMB(ctx.parsed.y) } } }, animation: { duration: 300 }, scales: { x: { display: false }, y: { beginAtZero: true, ticks: { color: '#7d8a99', font: { size: 9 }, callback: v => (v/1048576).toFixed(0)+'M' }, grid: { color: '#1c2530' } } } }
      });
      return memChart;
    }
    ...
    function applyStats(s) {
      ...
      if (s.system) {
        ...
        /* 内存趋势图：确保 canvas 存在后立即更新（使用 requestAnimationFrame 确保 DOM 已渲染） */
        requestAnimationFrame(() => {
          const mc = ensureMemChart();
          if (mc) {
            const data = memHistory.value.slice();
            mc.data.labels = data.map((_, i) => String(i));
            mc.data.datasets[0].data = data.length >= 2 ? data : [0, data[0] || 0];
            mc.update('none');
          }
        });
      }
    ...
    onMounted(() => {
      /* 提前初始化图表（使用 requestAnimationFrame 确保 Vue 渲染完毕后 canvas 已在 DOM 中） */
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          ensureRateChart();
          ensureCcChart();
  ...
  template: '<div class="fade-in">' +
    ...
    '<div style="max-width:1680px;margin:0 auto;padding:14px">' +
      ...
      '</div>' +
      '<div class="grid-3" style="display:grid;gap:12px;margin-top:12px">' +
        ...
        '<div class="card"><div class="card-title">元数据解析进度</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px"><div class="mini-stat"><div class="l">种子总数</div><div class="v">{{ fmt(stats.meta.total) }}</div></div><div class="mini-stat"><div class="l">已解析名</div><div class="v">{{ fmt(stats.meta.withName) }}</div></div><div class="mini-stat"><div class="l">已解析 size</div><div class="v">{{ fmt(stats.meta.withSize) }}</div></div><div class="mini-stat"><div class="l">聚合补全</div><div class="v" style="color:#d29922">{{ fmt(stats.counters.metaEnriched||0) }}</div></div></div><div style="margin-top:8px"><div style="font-size:10px;color:#54626f;display:flex;justify-content:space-between"><span>size 落库率</span><span style="color:#3fb950">{{ sizeRate.toFixed(2) }}%</span></div><div style="width:100%;height:5px;background:#232c38;border-radius:3px;overflow:hidden;margin-top:6px"><div style="height:100%;background:linear-gradient(90deg,#238636,#3fb950);transition:width 0.6s" :style="{ width: (sizeRate > 0 && sizeRate < 1 ? 1 : Math.min(100, sizeRate)) + \'%\' }"></div></div></div><div style="margin-top:6px;font-size:11px;color:#54626f">队列 {{ fmt(stats.metaQueue) }} · 元数据 OK {{ fmt(stats.meta.withMeta) }}</div></div>' +
      '</div>' +
      '<div class="grid-4" style="display:grid;gap:12px;margin-top:12px">' +
      ...
      '<div class="grid-events" style="display:grid;gap:12px;margin-top:12px">' +
        '<div class="card"><div class="card-title">实时事件流（最近 30 条）</div><div style="max-height:320px;overflow-y:auto"><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr><th>时间</th><th>IP</th><th>资源名</th><th>大小</th><th>来源</th></tr></thead><tbody><tr v-if="!stats.recent.length"><td colspan="5" style="color:#54626f;text-align:center;padding:20px;font-size:12px">暂无数据</td></tr><tr v-for="(e,i) in stats.recent" :key="e.ts + \'_\' + e.ip + \'_\' + i"><td class="mono-td">{{ fmtTime(e.ts) }}</td><td class="mono-td">{{ esc(e.ip) }}</td><td :title="esc(e.name)">{{ esc(e.name && e.name.length > 28 ? e.name.slice(0,28) + \'…\' : (e.name || e.infohash.slice(0,12) + \'…\')) }}</td><td class="mono-td">{{ fmtSize(e.size) }}</td><td><span :class="\'src-badge src-\' + e.source">{{ e.source }}</span></td></tr></tbody></table></div></div>' +
        '<div class="card"><div class="card-title">DHT 路由表节点 <span style="font-size:10px;color:#54626f">{{ nodes.length ? \'(\' + fmt(nodes.length) + \')\' : \'\' }}</span></div><div style="max-height:320px;overflow-y:auto"><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr><th>Node ID</th><th>地址</th><th>族</th><th>年龄</th></tr></thead><tbody><tr v-if="!nodes.length"><td colspan="4" style="color:#54626f;text-align:center;padding:20px;font-size:12px">{{ stats.dht.running ? \'暂无节点\' : \'DHT 未运行（sim 模式不启动 DHT，使用 --live 启动）\' }}</td></tr><tr v-for="n in nodes" :key="n.id + \'_\' + n.address"><td class="mono-td">{{ n.id }}</td><td class="mono-td">{{ n.address }}</td><td>{{ n.family === \'ipv6\' ? \'v6\' : \'v4\' }}</td><td>{{ n.ageSec }}s</td></tr></tbody></table></div></div>' +
        '<div class="card"><div class="card-title">系统资源</div><div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px"><div class="mini-stat"><div class="l">内存 RSS</div><div class="v">{{ fmtMB(stats.system.rss) }}</div></div><div class="mini-stat"><div class="l">堆使用</div><div class="v">{{ fmtMB(stats.system.heapUsed) }}</div></div><div class="mini-stat"><div class="l">运行时长</div><div class="v">{{ fmtDur(stats.system.uptime) }}</div></div><div class="mini-stat"><div class="l">Node 版本</div><div class="v" style="font-size:12px">{{ stats.system.nodeVersion }}</div></div></div><div style="margin-top:8px"><div style="font-size:10px;color:#54626f">内存占用趋势 <span style="color:#d29922">{{ fmtMB(stats.system.rss) }}</span></div><div style="position:relative;height:80px;margin-top:4px"><canvas id="memChart"></canvas></div></div></div>' +
      '</div>' +
    '</div>' +
